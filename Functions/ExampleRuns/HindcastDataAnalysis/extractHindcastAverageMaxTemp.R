...
## PARKERSBURG MAXTEMP ANALYSIS -----
location <- "Parkersburg, WV"

# Parkersburg Historical Average
...
for (c in 1:length(goodmodelnames)) {
  ...
  hindcast.maxtemp.poly <- mean(sapply(filelist.vec,extractLocationAttribute,
                                 location="Parkersburg",shape="polygon"))
  hindcast.maxtemp.point <- mean(sapply(filelist.vec,extractLocationAttribute,
                                  location="Parkersburg",shape="point"))
  
  #create vector to bind to temporary data frame
  vec.to.bind <- c(goodmodelnames[c],hindcast.maxtemp.point,hindcast.maxtemp.poly)
  #bind
...
write.csv(pb.tasmax.hindcast,
          file="~/2023/NEX-GDDP/Code_Larsson/Analysis/hindcastData/ModelOutputs/ParkersburgAverageMaxTemp.csv")


## CHARLESTON MAXTEMP ANALYSIS -----
...
for (c in 1:length(goodmodelnames)) {
  ...
  #polygon and point extractions
  hindcast.maxtemp.poly <- mean(sapply(filelist.vec,extractLocationAttribute,
                                       location="Charleston",shape="polygon"))
  hindcast.maxtemp.point <- mean(sapply(filelist.vec,extractLocationAttribute,
                                        location="Charleston",shape="point"))
...
write.csv(ch.tasmax.hindcast,
          file="~/2023/NEX-GDDP/Code_Larsson/Analysis/hindcastData/ModelOutputs/CharlestonAverageMaxTemp.csv")


## BLUESTONELAKE MAXTEMP ANALYSIS -----
...
for (c in 1:length(goodmodelnames)) {
  ...
  #polygon and point extractions
  hindcast.maxtemp.poly <- mean(sapply(filelist.vec,extractLocationAttribute,
                                       location="BluestoneLake",shape="polygon"))
  hindcast.maxtemp.point <- mean(sapply(filelist.vec,extractLocationAttribute,
                                        location="BluestoneLake",shape="point"))
...
write.csv(bl.tasmax.hindcast,
          file="~/2023/NEX-GDDP/Code_Larsson/Analysis/hindcastData/ModelOutputs/BluestoneLakeAverageMaxTemp.csv")
